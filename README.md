...
| 模型提问（实验性） | 模型可在交互回合中暂停并显示一个 2–4 项的单选问题；按数字选择后，答案作为普通工具结果继续当前回合 |
...
当模型确实缺少一个必须由你决定的信息时，可以调用 `ask_user_question`。当前终端版本
一次只显示一个问题，提供 2–4 个单选项；增强界面直接按对应数字，线性界面输入数字后
按 Enter。问题不会替模型批准文件或 Shell 操作，也不会改变现有审批规则。批量问题、
自由文本回答和多选仍未实现。
