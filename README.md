...
| 模型提问（实验性） | 模型可在交互回合中暂停并顺序显示 1–3 个问题，每个问题有 2–4 个单选项；全部按数字回答后继续当前回合 |
...
3 个问题，终端按顺序逐个显示，每题提供 2–4 个单选项；增强界面直接按对应数字，线性
界面输入数字后按 Enter。问题不会替模型批准文件或 Shell 操作，也不会改变现有审批
规则。自由文本回答和多选仍未实现。
