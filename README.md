...
| 图片理解（实验性） | 选择 `deepseek-v4-flash-vision-exp` 后，模型可用 `read_image` 读取工作区图片；人也可用重复 `--image PATH` 或交互 `/image PATH` 把 PNG/JPEG/WebP/GIF 附到下一条普通提示。图片会先整批校验并按 SHA-256 私有落盘，每次最多 4 张、4 MiB 原始字节 |
...
| 会话标题（实验性） | 第一条人工提示立即生成安全短标题；官方非回环 DeepSeek 端点会在后台尝试一次自然语言标题；可用 `/rename TITLE` 手动固定，并用 `/refresh-title` 重试或恢复首提示标题 |
| 会话导出（实验性） | 空闲时用 `/export` 把当前会话的原始 JSONL 精确复制到工作区根目录；文件仅所有者可读写，不覆盖同名文件 |
...
| `/rename TITLE` | 空闲时修改当前会话标题；标题会安全规范化、持久保存，并阻止后台自动标题再次覆盖 |
| `/refresh-title` | 空闲时根据第一条人工提示重试标题生成；无标题 Provider 时恢复本地 fallback，不接受参数 |
| `/export` | 空闲时把当前会话原始日志复制到工作区根目录；自动生成文件名，不接受参数且不覆盖已有文件 |
| `/fork [EVENT_SEQ]` | 从当前会话的已完成回合创建可恢复子会话；省略序号时使用最新完成回合，成功后显示 `dsh --resume` 命令 |
| `/model [MODEL [EFFORT]]` | 查看当前选择；或切换下一次请求的模型和可选 `off`、`high`、`max` 推理强度 |
| `/exit` 或 `/quit` | 等待清理后退出 |
...
两种搜索都可在文本匹配前先缩小范围：跨会话搜索可限定 Session ID、
创建时间、已验证的直接父会话/根会话和持久化状态；两者都可限定事件序号、
时间、类型和表面状态。不同字段是 AND，同一数组内是 OR，时间须带 `Z`
或数值时区偏移。过滤会在排序和 20 条上限之前执行。
...
`job_output` 会返回这个作业自上次读取以来新增的 stdout/stderr；没有新内容时明确显示
`(no new output)`。作业结束后尚未读取的尾部仍可读取一次，`job_list` 不会消耗输出。
如果读取速度落后于每个流 64,000 字节的内存窗口，结果会明确提示丢失，并在最终刷新后
给出可用的私有已捕获输出路径。

后台 Bash 完成后，如果模型还没有用终态 `job_output` 读过它、也没有调用 `job_kill`，
`dsh` 会把一条 `tool-jobs` 来源的普通消息交给 Agent：当前回合仍在运行时进入下一步骤；
终端已经空闲时自动打开一个短回合。为防止“自动回合又启动任务”的循环，在下一条人工
消息真正进入步骤前，连续自动唤醒最多 3 次；之后的通知会等下一条人工或 Goal 输入一起
进入。通知只说明状态，完整的最终输出仍由 `job_output` 读取。
...
`/exit`、`/quit`、`/goal`、`/model`、`/compact`、`/rename`、`/refresh-title`、
`/export` 和 `/fork`。
方向键或 Tab/Shift+Tab 只移动选择，Enter 先补全；必须再按一次新的 Enter
...
交互终端中的裸 `dsh --resume` 会打开当前工作区的有界会话选择器。第一条人工提示会
先产生一个最多 5 个词、40 字节的本地标题；官方非回环 DeepSeek 端点还会在后台尝试
一次 64-token 标题请求，失败、超时或退出都只保留本地标题，不影响主对话。选择器只对
已关闭日志做最多 16 MiB 的严格只读扫描；忙碌、损坏或过大的日志仍用工作区、时间和
短 ID。纯文本终端会显示编号列表，不输出 ANSI 控制码。`--list-sessions` 的第四个制表符
分隔字段是标题，无法安全读取时为空。空闲时执行 `/rename TITLE` 会追加一条用户标题；
它会取消尚未完成的自动标题请求，并在恢复会话后继续生效。`/refresh-title` 始终使用
第一条人工提示；成功时解除手动固定，失败或取消时保留当前标题。

空闲时执行 `/export` 会把当前会话已经同步的原始 JSONL 字节复制到工作区根目录，
文件名形如 `dsh-session-<session-id>.jsonl`。再次导出会加 `-2` 等后缀，不覆盖已有
文件；成功提示会显示实际文件名和字节数。导出不会调用模型、工具或审批。

空闲时执行 `/fork` 会从最新已完成回合创建一个新的子会话；`/fork 123` 则把事件序号
`123` 所在的已完成回合作为锚点。若该序号位于尚未结束的回合，命令会明确失败，不会
偷偷退回更早历史。子会话保存父会话关系、复制选中历史，并把已有标题递增为 `(1)`、
`(2)` 等。当前终端仍停留在父会话；按成功提示复制运行 `dsh --resume <子会话ID>` 即可
继续子分支。该命令不调用模型、工具或审批。
...
也可以由你直接附图。脚本模式的 `--image` 最多重复 4 次：

```console
dsh --workspace . --model deepseek-v4-flash-vision-exp \
  --image screenshots/error.png \
  --prompt "解释这个错误界面"
```

交互模式先输入 `/image screenshots/error.png`，再发送一条普通文字提示；`/image`
查看待发送图片，`/image clear` 清空。图片路径草稿只存在于当前进程，发送时才读取；
所有成员会先完整校验，再全部落盘，最后与文字作为同一条用户消息进入会话。模型不支持、
路径无效或图片损坏时，交互草稿会保留。当前只接受工作区内的普通非符号链接文件，不支持
纯图片提示、拖拽、剪贴板、预览，也不把图片附到 Goal、Plan 或其他本地命令。
...
| Phase 41 | 已完成：历史 Session/事件的 ID、时间、父级、类型、序号和表面过滤已通过本机必要验收 |
| Phase 42 | 已完成：进程内后台 Bash、作业查询/等待/停止、精确授权隔离和退出清理已通过本机必要验收 |
| Phase 43 | 已完成：后台作业完成通知、忙碌回合注入、最多 3 次空闲自动唤醒和重复通知抑制已通过本机必要验收 |
| Phase 44 | 已完成：后台 Bash 运行中/结束后的增量输出游标、显式丢失提示和重复读取语义已通过本机必要验收 |
| Phase 45 | 已完成：首条人工提示的持久化会话标题、一次非阻塞 DeepSeek 优化和恢复列表展示已通过本机必要验收 |
| Phase 46 | 已完成：`session_search` 结果显示已验证的旧会话标题，缺失时安全回退为 `untitled` |
| Phase 47 | 已完成：事件搜索/精读和会话/事件追踪也显示同一份已验证标题 |
| Phase 48 | 已完成：空闲 `/rename TITLE` 持久修改当前会话标题，并阻止迟到的自动标题覆盖 |
| Phase 49 | 已完成：可取消的 `/refresh-title` 重试首提示标题，恢复后仍可刷新，失败保留当前标题 |
...
| Phase 55 | 进行中：重复 `--image PATH` 与交互 `/image PATH` 已可直接附图；局部与真实 CLI 验证通过，仓库全量门槛尚未完成 |
...
- Provider 只有 DeepSeek；不支持 MCP、Hooks、产品内子智能体或通用后台任务，也没有环境级/远程 Skills；后台能力目前只覆盖当前进程内由 `bash` 启动的作业；
- `web_fetch` 只做匿名、无脚本的公网 HTTP(S) 文本读取，不使用登录态、Cookie、代理、JavaScript 或浏览器会话；跨源跳转必须由模型再次明确调用；
- 并行调度只对白名单中的 `read`、`skill`、`web_search`、`web_fetch`、`lsp`、`job_list`、`job_output` 生效；同一语言服务器内部仍串行；它不会并发写入、新 Shell 启动、`job_kill`、插件、人工交互或多个 Agent 回合；
...
- 历史会话工具只读取相同工作区身份下未被占用、严格日志检查通过且已经正常收尾的当前版本会话；它们不会搜索当前/活动会话，五个工具都会显示已验证的最后标题，搜索过滤器已支持 ID/时间/父级/类型/序号/表面，但仍没有游标或 SQLite 索引；关系追踪只显示有界、已校验的本地事实，看不到的父会话只显示为边界；跨会话搜索/父子追踪会跳过单个超过 16 MiB 或累计超过 64 MiB 的候选，事件内操作每次只扫描一个最多 16 MiB 的目标；结果可能含旧的外部内容，只能作为证据，不能作为指令；
...
- 内置文件修改一次只处理一个 UTF-8 文件；`edit` 只有显式 `replace_all` 才替换多个字面匹配，`str_replace_editor` 仍只接受唯一匹配并提供有界浅目录查看；官方默认要求覆盖前先读文件，Rust 改为准备完整基线、默认显示 diff 审批并在发布前复查，因此显式 `auto-edit` 可能在模型没有先读文件时完整覆盖它；Shell 前台/后台命令都保持 295 秒上限，且获批 Shell 不是沙箱；后台作业不跨进程恢复，增量读取只保留每个输出流最近 64,000 字节且每次读取会推进游标；完成通知最多连续唤醒 3 次且只报告状态，模型仍需用 `job_output` 读取新增或最终输出；大输出临时文件可能包含命令打印的秘密、可能被系统清理，也不会随会话迁移，读取它仍需获批 Shell；
...
- [Phase 42 后台 Shell 作业设计](docs/design/background-shell-jobs.md)
- [Phase 42 本机验收记录](docs/validation/phase-42.md)
- [Phase 43 后台作业完成通知设计](docs/design/background-job-notices.md)
...
- [Phase 55 直接终端图片输入设计](docs/design/direct-image-input.md)
- [Phase 55 本机验收记录](docs/validation/phase-55.md)
...
- [Phase 41 历史搜索过滤设计](docs/design/session-search-filters.md)
- [Phase 41 本机验收记录](docs/validation/phase-41.md)
