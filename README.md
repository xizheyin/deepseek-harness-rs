...
| 文件修改 | 严格的单文件 `apply_patch`，以及官方风格的 `str_replace_editor` 查看、创建、唯一文本替换和按行插入；两者都检查实际 diff、路径、符号链接和并发修改，默认审批，也可显式启用进程级 `auto-edit` |
...
- `apply_patch` 和 `str_replace_editor` 一次只处理一个文件；精确编辑只支持 UTF-8 文本、唯一字面匹配和有界浅目录查看；Shell 只运行有界的前台命令，且获批 Shell 不是沙箱；大输出临时文件可能包含命令打印的秘密、可能被系统清理，也不会随会话迁移，读取它仍需获批 Shell；
