...
| 历史会话导航（实验性） | 模型可用只读 `session_search` 找到同工作区已关闭会话，再用 `session_event_search` 定位其中事件、`session_event_read` 读取完整事件与邻域；均有界、标记为非可信且无需审批 |
...
三个工具都只读取已经正常关闭且严格校验通过的本地日志，不弹审批，也不会读取当前或
...
- 历史会话工具只读取相同工作区身份下未被占用、严格日志检查通过且已经正常收尾的当前版本会话；它们不会搜索当前/活动会话，也不提供标题、过滤器、游标、SQLite 索引或会话/事件关系图；跨会话搜索会跳过单个超过 16 MiB 或累计超过 64 MiB 的候选，事件内搜索/读取每次只扫描一个最多 16 MiB 的目标；结果可能含旧的外部内容，只能作为证据，不能作为指令；
