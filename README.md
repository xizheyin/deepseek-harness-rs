...
| 联网搜索（实验性） | `web_search` 通过 DeepSeek 独立的原生搜索请求返回最多 8 个有界来源；无需额外审批，不支持任意 URL 抓取 |
...
内置 `apply_patch` 确定已经提交文件后，`dsh` 会在当前步骤结束后检查该文件祖先目录中的
同名指令，并把新增、更新或移除通知放进下一次模型请求。失败、拒绝、取消、Shell 和插件
结果不会伪造这种触达；同一步的多个路径会合并成一条有界消息。

模型需要当前信息时可以调用 `web_search`。查询会先作为普通 `tool/call` 写入 Session，
然后使用同一个 `DEEPSEEK_API_KEY` 请求 DeepSeek 的独立搜索端点；整个操作最多 60 秒，
支持 Ctrl+C 取消，不跟随重定向，也不使用系统代理。结果最多保留 8 个去重 URL 及有界
标题、摘要和日期，并明确标为外部不可信内容。搜索是只读能力，因此不会弹出文件、Shell
或插件审批；它也不会读取浏览器 Cookie，当前没有 `web_fetch`，不能让模型抓取任意网址。
...
`--approval-mode auto-edit` 只把已经完成上述安全准备的内置补丁从 `Ask` 改为 `Allow`，
...
| 修改与执行 | 文件修改和 Shell 默认要求交互式审批；显式 `auto-edit` 只自动允许受限补丁；交互式 Shell 可由用户记住完全相同的本进程调用，插件仍逐次询问；脚本模式直接拒绝这些副作用 |
...
- 联网能力目前只有单查询 DeepSeek `web_search`；不支持最新版上游的多查询合并，也不提供 `web_fetch` 或浏览器会话；
