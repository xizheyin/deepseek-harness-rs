...
use super::{
    EventKind, SessionEvent, SessionId, SessionStore, StoreError, TodoStatus, TurnEndReason,
    recovery::scan_jsonl_observing,
};
...
async fn settle_worker(
    worker: tokio::task::JoinHandle<Result<SessionSearchOutcome, SessionSearchError>>,
) -> Result<(), SessionSearchError> {
    match worker.await {
        Ok(_) => Ok(()),
        Err(_) => Err(SessionSearchError::Unavailable),
    }
}

fn map_worker_result(
    result: Result<Result<SessionSearchOutcome, SessionSearchError>, JoinError>,
) -> Result<SessionSearchOutcome, SessionSearchError> {
    result.map_err(|_| SessionSearchError::Unavailable)?
}
...
mod tests {
    ...
    use crate::{
        ...
        session::{
            EventSeq, NewEvent, Session, SessionHeader, SurfaceIntent, TurnEndReason, TurnId,
            UnixMillis,
        },
